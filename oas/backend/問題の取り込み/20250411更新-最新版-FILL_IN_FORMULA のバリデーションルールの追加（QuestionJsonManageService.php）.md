...
2. **field_id の重複チェック**
    - すでに登場した `field_id` と同じものが含まれていないことを検証。
3. **attribute の存在**
    - `attribute` キーが指定されていない場合はエラーにしている。
4. **attribute の型(Enum)チェック**
    - `QuestionMetadataInputFormatFieldAttribute::fromString($attrRaw)` で変換が成功するかを確認し、失敗した場合はサポート対象外としてエラー。
5. **user_answer の存在および文字列チェック**
    - `user_answer` が存在し、かつ文字列かどうかを検証し、必須であるかをチェックしている。
6. **user_answer の型(Enum)チェック**
    - `FieldType::from($f['user_answer'])` を実行し、`number`, `string`, `operator`, `sequence` などの有効な値かを確認。
    - Enumに無い文字列が渡された場合はエラーとしている。
7. **collect_answer が含まれていないこと**
...
・ソースコードが冗長になって長くなってもよいので、１つのチェックに複数のタイプの条件を付け加えるのではなく、同じチェック項目内容であってもタイプ毎に条件を分けてチェックする方針で実装してほしい
...
metadata.input_format.input_components: metadata.question_type が、FILL_IN_OPERATOR または、FILL_IN_MULTIPART　の時は必須。（回答の選択肢。入力キーパッドのボタン）FILL_IN_MULTIPART　の時は、
...
class ImportQuestionsFromGithub extends Command
{
     ...
     * artisan で呼び出すコマンド名
     *
     * 例: php artisan import:questions-from-github --path=math/g3/sec300/u300_あまりのあるわり算/diff100/FILL_IN_MULTIPART/v1.0.0
    ...
    public function handle()
    {
        ...
        // status != PUBLISHED の問題をコンソール出力
        $this->showNonPublishedQuestions();
    ...
    private function importFromGitHub(): void
    {
        ...
        // path オプション が指定されている場合は対象のパスのみを取得する
    ...
    /**
     * 処理後に questions のデータで status != PUBLISHED をコンソールに出力する.
     * コンソールには json_id と status(文字列) を出力する.
     */
    private function showNonPublishedQuestions(): void
    {
        $nonPublishedQuestions = Question::where('status', '!=', QuestionStatus::PUBLISHED->value)->get();
        if ($nonPublishedQuestions->isNotEmpty()) {
            $this->info("==== Non-PUBLISHED questions ====");
            foreach ($nonPublishedQuestions as $q) {
                $statusLabel = QuestionStatus::from($q->status)->label();
                $this->line("json_id={$q->json_id}, status={$statusLabel}");
            }
        }
    }
}
...
use App\Enums\QuestionStatus;
use App\Enums\QuestionType;
use App\Enums\InputComponentType;
use App\Models\Question\Question;
use App\Models\Question\QuestionTranslation;
use Illuminate\Support\Facades\DB;
use Illuminate\Support\Str;
        ...
        if ($questionTypeValue === QuestionType::FILL_IN_OPERATOR->value) {
            ...
            foreach ($inputComps as $idx => $ic) {
                ...
                if (!$typeVal) {
                    ...
                    if (!InputComponentType::tryFrom($typeVal)) {
                        $validator->errors()->add("{$icKey}.{$idx}.type",
                            "不正な type='{$typeVal}' です。"
                        );
                    }
