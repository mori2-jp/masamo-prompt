ありがとう。
以下は QuestionJSON の中身をバリデーションするとコードです。
collect_answer のルールについて、
現在のルールに加えて、
collect_answer も、オブジェクトになっており、LANGUAGES　に存在しているキーが存在するかチェックしてほしい。
また、オブジェクトの中の値については現在の collect_answer のルールを適用してほしい。

また
metadata.input_format.fields.collect_answer: 絶対に存在してはいけない。ユーザーに回答が見えてしまうため。
このルールも追加してほしい

※変更箇所以外は、既存の実装をそのままにしてください！！！！
