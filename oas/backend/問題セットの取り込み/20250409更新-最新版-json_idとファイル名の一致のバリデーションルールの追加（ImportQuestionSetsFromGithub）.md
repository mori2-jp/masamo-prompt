以下、ImportQuestionSetsFromGithub.php　の処理に新しいバリデーションのルールを追加してください。
修正箇所を知らせてください。
...
# 追加するルール
インポートしたファイルの json_id と ファイル名が一致しているかチェックしてほしい。
例えば、
qset_s1_g3_sec700_u300_v100_700.json
というファイル名であれば、
JSON内の、json_id は、ファイル名から、.json を除いた qset_s1_g3_sec700_u300_v100_700　でなければいけません。
ファイル名とjson_idが不一致の場合はバリデーションエラーとしてDBに取り込みません。
処理の最後に、ファイル名とjson_idが不一致だったファイル名とjson_idをコンソールに全て出力してください。
...
class ImportQuestionSetsFromGithub extends Command
{
    ...
    /**
     * 【追加】JSON_NOT_MATCHED に更新された question_set の総数
     */
