...
# 条件
・No1についての問題を作成してください
数を順番に数えられる について、例題のように 1 から 10 までの1つのランダムの数字で絵文字を使って選択された数を答える問題を作成してください。 question は1問だけね。
...
・「✏️ はひだりから □ ばんめ」のように、問題文にも絵文字が含まれると回答者が問題文の絵文字を数えてしまい混乱します。「えんぴつはひだりから □ ばんめ」のように問題文には絵文字を必ず含めないこと。
・回答箇所の絵文字以外は、全て同じ絵文字を利用すること
・🍎🍇🍊の場合は食べ物として扱ってください。動詞を「たべる」にするなど
・絵文字を横に並べるときは、**必ず各絵文字の間に半角スペースを 1 つ入れてください**（例：🐶 🐶 🐶）。
...
・"最重要”作問済みの問題と重複する絵文字の問題は作問しないでください。（この一文は、generate_prompt に含めないこと）「りんごは例です。絵文字は｛# 使用可能絵文字一覧｝からランダムに選んでください」
...
1. 🍎 🍎 🍎 ✏️ 🍎 🍎 🍎 🍎 🍎 🍎 の列（text, order: 50）
2. newline
3. えんぴつはひだりから（text, order: 100）
5. 入力欄（input_field order: 150）
4. ばんめ（text, order: 200）(あれば）
