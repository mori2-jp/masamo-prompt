...
## 目的
指定の規則に従って、複数の JSON ファイルを作成し、
`contents/questions/math/g1/sec100/u200/codex` 直下へ
ディレクトリを追加せず並列に保存してください。

## 変数（必要なら値を変更）
BASE_DIR          = "contents/questions/math/g1/sec100/u200/codex"
FILE_PREFIX       = "ques_s1_g1_sec100_u200_diff100_qt51_v100_"
FILE_COUNTER_INIT = 4300   # 末尾番号の開始値
FILE_COUNTER_STEP = 100    # 末尾番号の増分

## 数列生成ルール
- `start_num` を **51** から始め、**3 ずつ増やす**  
  (51, 54, 57, … と進め、**125 を超えたら終了**)
- 各 `start_num` について  
  `end_num = start_num + 4` として 5 連続整数を得る

## JSON 生成
各ペア `(start_num, end_num)` ごとに、  
**内部プロンプト** を使って JSON を 1 つ生成してください。  
（必要に応じて埋め込みや置換を行ってください）

## ファイル保存ルール
1. ループの 0 回目は  
   `FILE_PREFIX + "4300.json"`
2. 以降は 1 ループごとに末尾番号を **100 ずつ加算**  
   例: 4300 → 4400 → 4500 → …
3. 生成した JSON は **UTF-8** で  
   `BASE_DIR/<ファイル名>` に保存する  
   （サブフォルダは作らない）

## 完了判定
- `start_num` が 51→54→57→… と進み、  
  **125 を超えた時点** でループを終了し、  
  すべてのファイルが保存されていれば完了。
...
2 位数，簡単な 3 位数の読み書き・比較に関する「数の順番当て問題」を **1 問だけ** 作成します。  
★start_num と ★end_num で指定した **5 個連続の整数を 1 行目** に，その数列を **すべて +1 した数列を 2 行目** に並べます。  
（例：★start_num=11, ★end_num=15 のとき  
　1 行目 → 11 12 13 14 15  
　2 行目 → 12 13 14 15 16）
...
1. 各行の 5 個の整数を **「半角スペース＋→＋半角スペース」で区切って表示** する。
2. **空欄 (□)** は 1 行目・2 行目に 1 か所ずつ，次のルールで決める。
    - まず 1 行目の 5 位置 (先頭＝1 番目 … 末尾＝5 番目) から 1 つを **等確率 (各 20 %)** で選び，その位置を □ にする。
    - 続いて 2 行目でも **残り 4 つ**の位置から 1 つを **等確率** で選び，□ にする。
    - **2 つの空欄位置は必ず異なる**。
    - ただし **ヒントが消えるのを防ぐため**，1 行目の ★start_num と 2 行目の (★end_num+1) に対応する位置は **空欄にしてはいけない**。
3. 表示形式
...
5. **負の数・分数は使用しない**。
6. `question_text` と `question_components` だけで UI を構築すること。
...
【例題 (★start_num=11, ★end_num=15 の一例／空欄位置は毎回ランダム)】  
11 → 12 → □ → 14 → 15  
12 → 13 → 14 → □ → 16
