...
# 条件
・No3についての問題を作成してください
加法及び減法の意味 について、例題のような 文章問題を絵文字を使った数式に変換して回答をさせる回答が１位数になるたし算　の問題を作問してください。 question は1問だけね。

【必須構成】
1. 日本語・英語の **文章問題**（例：こうえんに～？ / In the park, ...?）
2. 同じ内容を **絵文字式** に変換した行
   └ ⚠️ かならず （🍎🍎）+（🍎）＝□ のような書式
3. 正解が一桁になる **引き算**
4. 使用絵文字はこの一覧のみ → 🍎 🐶 🐟️ 😺 🍊 🍇 🕊
5. 「evaluation_spec.response_format.explanation」に
   *なぜその答え* になるかを ja / en で記入
6. マイナス解答・分数・繰り下がりは禁止

【例題】
こうえんに、とり🕊が３わいます。あとから２わとんできました。こうえんにとりはなんわ？
（🕊🕊🕊）+（🕊🕊）＝□ わ
