...
分数　の意味の理解　について、例題のように、分数をN倍したときの分数を答えさせることで分数理解させる問題を作成してください。
問題の形式と回答の数は例題と全く同じで例題の数字を変更する形で作問してください。
分数の意味を理解させたいので、約分をしたくないです。1を10等分したうちの5こ 5/10 を、1/2 のような約分が出来る問題にしないでください
ただし、小学３年生向けなので回答が帯分数にならない数にしてください。、question は1問だけね。
例題：・1/3 + 1/3 =

以下の間違いのように１つの分数に対して回答の空欄を分子分母の２つの解答欄に分けることはせず、必ず１つの分数に対して解答欄は分子分母をあわせた１つだけにしてください
間違い：1 を 8等分した 3こ分を分数で表すと、□ / □ です。
正しい：1 を 10等分した 7こ分は □ です

background と collect_answer 以外の「全て」の分数の表記は全て KateX を使ってください
...
正答が分数になる場合は、evaluation_spec の fields は
"user_answer": "string",
として
"1/7"
のように文字列としてください。
