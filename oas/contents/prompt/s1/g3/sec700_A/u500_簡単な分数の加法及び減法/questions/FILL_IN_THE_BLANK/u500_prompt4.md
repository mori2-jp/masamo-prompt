...
分数どうしの3つの足し算を問う問題を作問して。 question は1問だけね。
例）・1/7 + 2/7 + 3/7 = □
回答は約分できない数値にしてください
...
・分数の KateX 表記
分数は全て KateX で表記してください（日本語（ja）、英語（en）どちらも例外なく出力するJSONに登場する分数全てです）
分数は必ずインライン数式で記述してください。$$\frac{3}{4}$$ は ディスプレイ数式 (display math) と呼ばれる書き方。(\frac{1}{4}) は インライン数式 (inline math) と呼ばれる書き方。
KateXでは最終的に1つのバックスラッシュを見せたいです。JSON では、文字列リテラルの中でバックスラッシュ（\）をそのまま書くことができないため、バックスラッシュ自体をエスケープ（\）する必要があります。 
\u005C のようにバックスラッシュに文字参照は使わずに、JSON上にはバックスラッシュを２つ書くようにしてください。
