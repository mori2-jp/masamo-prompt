...
・No45（簡単な分数の加法及び減法）についての問題を作成してください

分数　（分数は全て KateX で表記してください）
分数どうしの引き算を問う問題を作問して。 question は1問だけね。
例）\(\frac{8}{10}\) - \(\frac{1}{10}\) = □
・分数は問題文、問題、正答など日本語、英語でも全て KateX で表記してください
回答は約分できない数値にしてください
...
collect_answer は KateX は使わずに、2/5 などの表記としてください。
