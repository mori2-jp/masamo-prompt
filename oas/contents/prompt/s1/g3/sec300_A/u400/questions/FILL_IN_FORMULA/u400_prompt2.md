...
- **evaluation_spec.checker_method**
  ...
  - 例: `"CHECK_BY_SINGLE_FORMULA_MATCH"`
- **evaluation_spec.response_format**
  ...
  - **fields**:
    ...
    - `fields` 配列には、ユーザーの各解答欄ごとの正誤情報や正解データを定義し、**field_id** で **metadata.input_format.fields** と対応付けます。
      - **field_id**: `"f_1"`, `"f_2"` など。
      - **user_answer**: ユーザー入力値の型を表し、`"array"` などを指定（実際のユーザー入力そのものはフロント側で受け取ります）。FILL_IN_FORMULA では、式と回答のセットの組み合わせになる。式は "array" で固定
      - **is_correct**: `"boolean"` 文字列固定。個々のフィールドが正解か否か。
      - **collect_answer**: 多言語オブジェクトで、正解となる値を `{ja, en}` の各言語で示します。Arrayなら `{"ja": ["12","÷","3"], "en": ["12","÷","3"]}` のように式の各値が配列で格納され、数値なら `{"ja": 4, "en": 4}` のように式を解いた回答が格納されています。FILL_IN_FORMULA では、必ず、式と回答のセットになるので、式となる array が必ず１つ以上入っています。実際にはユーザーには見せず、フロント側・CheckerMethodなどで使われます。
...
- **metadata.question_type**
  - 問題の形式を表し、"FILL_IN_FORMULA" などが入ります。
...
- **metadata.input_format**
  ...
  - **fields**: ユーザーが回答を入力する欄の定義を配列でまとめます。
    ...
    - **attribute**: `"array"` などの属性。FILL_IN_FORMULA は、array で固定。
    - **user_answer**: "array" など、実際の回答の型を指定。FILL_IN_FORMULA は、array で固定。　**collect_answer** はここには絶対に含めない（ユーザーに答えが見えてしまうため禁止）。
...
62	小3	A 数と計算	プログラミング	プログラミングの基礎	sec_s1_g3_1600	順次処理の体験	u100	・簡単な指示（例:キャラクターを前進→ジャンプ→回転）の手順を正しい順番で並べ想定どおりに動かせる。	・日常の手順やビジュアル型プログラミング環境を活用し順次処理の概念を楽しく学ばせる。
63	小3	A 数と計算	プログラミング	プログラミングの基礎	sec_s1_g3_1600	くり返し処理の体験	u200	・同じ動作を複数回くり返す命令を使いプログラムの効率化を図れる。	・簡単なゲーム要素や図形描画などを例に繰り返し命令の便利さと修正のしやすさを体感させる。
...
{
  "order": 100,
  "id": "ques_s1_g3_sec300_u400_diff100_qt401_v100_100",
  "level_id": "lev_003",
  "grade_id": "gra_003",
  ...
  "updated_at": "2025-03-24 18:00:00",
  "skills": [
    {
      "skill_id": "sk_004",
      "name": "知識・技能"
  ...
  "evaluation_spec": {
    ...
    "response_format": {
      ...
      "question": {
        "ja": "12個のあめを，1人に3個ずつあげると，何人に配れますか？",
        "en": "If you have 12 candies and give 3 to each person, to how many people can you distribute them?"
      },
      "fields": [
        {
          "field_id": "f_1",
...
# フォーマット説明
3. 式問題（FILL_IN_FORMULA）の例
   QuestionType::FILL_IN_FORMULA の場合、問題文中に空白部分（blank）があり、そこにユーザーが値を入力する形式になります。
...
3.2 サンプルJSON
以下は API から返却される例です。（"FILL_IN_FORMULA" の場合）
"question": {
...
ここで f_1 や f_2 といった field_id が回答フォームに対応し、ユーザーは数値や配列を入力します。
